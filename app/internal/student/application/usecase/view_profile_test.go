...
func TestViewExecuteWhenGetProfileByEmailFailShouldReturnError(t *testing.T) {
	...
	repositoryViewProfile.On("GetProfileByEmail", email).Return(domain.Profile{}, errors.New("")).Once()
	getUseCase := usecase.NewViewUseCase(
		repositoryViewProfile,
	)
	resultProfile, resultClasses, errResult := getUseCase.Execute(email)
...
func TestViewExecuteWhenGetClassesDoneByEmailShouldReturnError(t *testing.T) {
	profileDomain := *domain.NewProfile(
		email,
		name,
	)
	...
	repositoryViewProfile.On("GetProfileByEmail", email).Return(profileDomain, nil).Once()
	repositoryViewProfile.On("GetClassesDoneByEmail", email).Return([]domain.Class{}, errors.New("")).Once()
	getUseCase := usecase.NewViewUseCase(
		repositoryViewProfile,
	)
	resultProfile, resultClasses, errResult := getUseCase.Execute(email)
